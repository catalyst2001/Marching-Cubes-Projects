...
{
public:
	VertexBuffer() : m_pVerts(nullptr), m_Size(0), m_Index(0), m_Reserve(1) {}
	VertexBuffer(int startsize, int reserve) {
		m_pVerts = (Vector3 *)malloc(startsize * sizeof(Vector3));
		m_Size = startsize;
		m_Reserve = reserve;
		m_Index = 0;
	}
	...
	void AddVertex(Vector3 vert) {
		...
		if ((m_Index + 1) == m_Size) {
			forcealloc:
			m_Size += m_Reserve;
			m_pVerts = (Vector3 *)realloc(m_pVerts, sizeof(Vector3) * m_Size);
		}
		m_pVerts[m_Index] = vert;
		m_Index++;
	}

	inline Vector3 *GetVertices() { return m_pVerts; }
	inline unsigned int GetVerticesCount() { return m_Index; }
...
int main()
{
	...
	while (msg.message != WM_CLOSE) {
		...
		glDrawElements(GL_TRIANGLE_STRIP, mc3.triangles.size(), GL_UNSIGNED_INT, &mc3.triangles);
