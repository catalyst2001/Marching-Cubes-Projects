...
void HandleDisplay(void)
{
   ...
   __near = 0.1;
   __far = 2*(grid.Nx*grid.Dx+grid.Ny*grid.Dy+grid.Nz*grid.Dz);
   if (stereo)
      __near = camera.focallength / 5;

   /* Misc stuff */
   ratio  = camera.screenwidth / (double)camera.screenheight;
   radians = DTOR * camera.aperture / 2;
   wd2     = __near * tan(radians);
   ndfl    = __near / camera.focallength;
   ...
   if (stereo) {
      ...
      top    =   wd2;
      bottom = - wd2;
      glFrustum(left,right,bottom,top,__near,__far);
...
void DrawExtras(void)
{
   ...
   if (showgrid) {
	...
	//TODO: CreateSimpleSphere not found. Disabled. Replace to glutWireSphere
...
int ReadVolume(char *fname)
{
   ...
   if (strlen(fname) < 1) {
		//fn = fl_show_file_selector("Volume file","","", MB_OK);
		if (OpenFileDialog(NULL, fn, "", "All\0*.*\0Volume file\0*.vol*\0")) {
			if (fn == NULL || strlen(fn) <= 0)
				return(TRUE);
			if ((fptr = fopen(fn, "r")) == NULL) {
				MessageBoxA(NULL, "File Error", "Unable to open volume data file", fn);
				return(FALSE);
			}
		}
		return FALSE;
      ...
      if ((fptr = fopen(fname,"r")) == NULL) {
         MessageBoxA(NULL, "File Error","Unable to open volume data file",fname);
         status = FALSE;
			goto skip;
      }
   }
	...
	if (fscanf(fptr,"%d %d %d",&nx,&ny,&nz) != 3) {
		MessageBoxA(NULL, "File Read Error","Failed to read grid dimensions", MB_OK);
		status = FALSE;
      goto skip;
	}
   if (fscanf(fptr,"%lf %lf %lf",&dx,&dy,&dz) != 3) {
      MessageBoxA(NULL, "File Read Error","Failed to read grid size", MB_OK);
      status = FALSE;
      goto skip;
   }
	dmin = MIN(dx,dy);
	dmin = MIN(dmin,dz);
	if (dmin <= 0) {
      MessageBoxA(NULL, "File Read Error","Read illegal grid size", MB_OK);
      status = FALSE;
      goto skip;
	}
	dx /= dmin;
   ...
   if (fscanf(fptr,"%lf %lf %lf",&xmin,&ymin,&zmin) != 3) {
      MessageBoxA(NULL, "File Read Error","Failed to read grid lower corner", MB_OK);
      status = FALSE;
      goto skip;
   }
   if (fscanf(fptr,"%d %d",&datasize,&endian) != 2) {
      MessageBoxA(NULL, "File Read Error","Failed to read grid data type", MB_OK);
      status = FALSE;
      goto skip;
   }
	if (datasize != 8 && datasize != 16) {
		MessageBoxA(NULL, "File Read Error","Unsupported cell size", MB_OK);
		status = FALSE;
		goto skip;
	}
...
int SaveVolume(char *s)
{
   ...
   /* Get the file name */
	if (strlen(s) < 1) {
		//fn = fl_show_file_selector("Volume file","","", MB_OK);
		if (SaveFileDialog(NULL, fn, "", "All\0*.*\0Volume file\0*.vol*\0")) {
			if (fn == NULL || strlen(fn) <= 0)
				strcpy(fname, fn);
		}
		return(FALSE);
   ...
   if ((fptr = fopen(fname,"w")) == NULL) {
		MessageBoxA(NULL, "File Save Error","fname", MB_OK);
      return(FALSE);
	}
