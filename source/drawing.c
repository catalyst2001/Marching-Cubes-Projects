   ...
   for (k=0;k<grid.Nz+grid.Rz;k+=grid.Rz) {
      ...
      for (j=0;j<grid.Ny+grid.Ry;j+=grid.Ry) {
			...
			for (i=0;i<grid.Nx+grid.Rx;i+=grid.Rx) {
				...
				/* Choose the colour */
     			if (cmap != CONSTANTCOLOUR) 
     				colour = GetColour((double)value, (double)grid.themin,(double)grid.themax,cmap);
...
void DrawInfo(int xstart,int ystart)
{
	...
	glPushMatrix();
	glLoadIdentity();
	glOrtho(0.0, camera.screenwidth, 0.0, camera.screenheight, -1.0, 1.0);
...
void DrawIsoSurface(int cmap)
{
   ...
   if (cmap == CONSTANTCOLOUR) {
      ...
      colour = GetColour((double)grid.isolevel, (double)grid.themin,(double)grid.themax,cmap);
      glColor4f(colour.r,colour.g,colour.b,1.0);
   }
